...
function proposeTrade(game, fromId, toId, offerProps, offerMoney, requestProps, requestMoney) {
  ...
  for (const pid of offerProps) {
    if (game.properties[pid].ownerId !== fromId) return { success: false, message: 'You don't own a property' };
  }
  for (const pid of requestProps) {
    if (game.properties[pid].ownerId !== toId) return { success: false, message: 'They don't own a property' };
  }
